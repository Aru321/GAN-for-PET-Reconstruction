...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.encoder_layer4_down=nn.Sequential(
            VisionTransformer(kernel_size=3,stride=1,padding=1,in_chans=256,embed_dim=512,
                                              depth=2,num_heads=4,mlp_ratio=4))
...
class Discriminator(nn.Module):
    ...
    def forward(self,x):
        ...
        return x
#
def D_train(D: Discriminator, G: Generator, X, Y,BCELoss, optimizer_D,device):
    # 标签转实物（右转左）x:lpet y:spet
...
#
## 训练生成器 ##
def G_train(D: Discriminator, G:Generator, X, Y,BCELoss, L1, optimizer_G, device,lamb=100):
    #device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    #image_size = X.size(3) // 2
    x = X.to(device)   
    y = Y.to(device)   
...
if __name__ == '__main__':
    ...
    x=torch.randn(1,128,16,16,16).cuda()
    y=torch.randn(1,1,64,64,64).cuda()
    blk=VisionTransformer(kernel_size=3,stride=2,padding=1,in_chans=128,embed_dim=128,
                                              depth=1,num_heads=2,mlp_ratio=4,drop_rate=0.3).cuda()
    cal_flops, cal_params = profile(blk, inputs=x)
    print("flops:", cal_flops / 1e9, "\nparams:", cal_params / 1e6)
    res=model(x)
    res1=torch.cat([res,y],dim=1)
    res1=modeld(res1)
    print(res1.shape)
    '''
