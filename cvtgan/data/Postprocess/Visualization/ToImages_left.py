...
def ToImages(root,fix):
    ...
    for file in all_name:
        ...
        #
        folder = os.path.exists('./imgs_left/' +fix+ file)
        if not folder:
            os.makedirs('./imgs_left/' +fix +file) 
        for j in range(128):
            img = MatrixToImage(image[:, :, j])
            img.save('./imgs_left/' +fix+ file+'/'+ str(j) + '.jpg')
#
def ToImages1(root):
    all_names=[]
    for root, dirs, files in os.walk(root):
    ...
    for file in all_name:
        ...
        mask = np.zeros((128,128,128))
        #
        folder = os.path.exists('./imgs_left/' + file)
        if not folder:
            os.makedirs('./imgs_left/' + file) 
        for j in range(128):
            img = MatrixToImage(image[:, :, j])
            img.save('./imgs_left/' + file+'/'+ str(j) + '.jpg')
