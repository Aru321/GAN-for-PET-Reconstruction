...
class TwoCrossFormerBlocks(nn.Module):
    ...
    def forward(self,x):
        ...
        return x
#
class OneCrossFormerBlock(nn.Module):
    def __init__(self, dim, input_resolution):
        super().__init__()
        self.resolution=input_resolution
        self.block1=CrossFormerBlock(dim,input_resolution,4,rev=True,lsda_flag=0)
...
class Generator(nn.Module):#A Small Version which is more Memory Friendly
    def __init__(self):
        ...
        self.encoder_layer0_down=nn.Sequential(
            nn.Conv3d(1,64,3,2,1),#embedding
            nn.LeakyReLU(0.2),
            nn.BatchNorm3d(64),
            OneCrossFormerBlock(64,32),
            )#first conv embed

        #32*32*32
        self.encoder_layer1_down = nn.Sequential(
            nn.Conv3d(64,128,3,2,1),#embedding
            TwoCrossFormerBlocks(128, 16),
        )  # first conv embed

        #4*4*4
        self.encoder_layer2_down = nn.Sequential(
            nn.Conv3d(128,192,3,2,1),
            TwoCrossFormerBlocks(192, 8),
        )  # first conv embed
        #self.encoder_layer2_down = nn.Conv3d(128,192,3,2,1)
        #
        self.encoder_layer3_down = nn.Sequential(
            nn.Conv3d(192,256,3,2,1),
            TwoCrossFormerBlocks(256, 4)
        )
        #
        #下采样输出
        #
        ...
        self.decoder_layer1_up=nn.Sequential(
            nn.ConvTranspose3d(256,192,kernel_size=2,stride=2),
            OneCrossFormerBlock(192, 8))
        #8*8*8
        self.decoder_layer2=CrossUP(192,8)

        #self.resup2=ResBlock3D(256*2, 256)
        self.decoder_layer2_up=nn.Sequential(
            nn.ConvTranspose3d(192,128,kernel_size=2,stride=2),
            OneCrossFormerBlock(128, 16),
        )
        #16*16*16
        #
        self.decoder_layer3=CrossUP(128,16)
    ...
    def forward(self,x):
        ...
        de1=self.decoder_layer2(de0,en2)
        de1=self.decoder_layer2_up(de1)
        #print("d1:",de1.shape)
        del de0
        #
        #cat2=torch.cat([en1,de1],1)
        de2=self.decoder_layer3(de1,en1)
