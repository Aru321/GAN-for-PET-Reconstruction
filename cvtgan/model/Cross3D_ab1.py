...
class Generator(nn.Module):#A Small Version which is more Memory Friendly
    def __init__(self):
        ...
        self.fuse0=nn.Conv3d(128,64,3,1,1)
        #32*32*32
        ...
        self.fuse1 = nn.Conv3d(256,128,3,1,1)
        #4*4*4
        self.encoder_layer2_down = nn.Sequential(
            PatchEmbed(img_size=16, patch_size=[3, 5], in_chans=128, embed_dim=192),
        ...
        self.fuse2 = nn.Conv3d(384,192,3,1,1)
        #
        self.encoder_layer3_down=nn.Sequential(
            nn.Conv3d(192,256,kernel_size=3,stride=2,padding=1),
    ...
    def forward(self,x,mri):
        ...
        en0=self.fuse0(torch.cat([self.encoder_layer0_down(x),mri0],dim=1))#32*32*32
        #print("en0:",en0.shape)
        en1=self.fuse1(torch.cat([self.encoder_layer1_down(en0),mri1],dim=1))#16*16*16
        #print("en1:",en1.shape)
        #
        en2=self.fuse2(torch.cat([self.encoder_layer2_down(en1),mri2],dim=1))#8*8*8
        #print("en2:",en2.shape)
