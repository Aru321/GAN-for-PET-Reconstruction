...
class Generator(nn.Module):#A Small Version which is more Memory Friendly
    def __init__(self):
        ...
        self.encoder_layer0_down=nn.Sequential(
            PatchEmbed(img_size=64,patch_size=[3,5,7],in_chans=2,embed_dim=64),
        ...
        self.encoder_layer3_down=nn.Sequential(
            PatchEmbed(img_size=8, patch_size=[3, 5], in_chans=192, embed_dim=256),
        )
        #
        #下采样输出
        #
    ...
    def forward(self,x,mri):
        ...
        #
        mix=torch.cat([x,mri],1)
        #x=self.encoder_conv(x)
        en0=self.encoder_layer0_down(mix)
